...
class FileParser
{
    ...
    {
        if ($this->checkFileIsReadable($file) === false) {
            return false;
        }
    ...
    public function __construct($file)
    {
        ...
        if ($url) {
            if ($this->getCurlFile() === null) {
                $this->setCurlFile(date('Ym') . '.csv');
            }
    ...
    private function _parseUrl()
    {
        ...
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true); // Return the page
        ...
        $headerRow = isset($data[0]) ? $data[0] : NULL;
        if (isset($data[0])) {
            unset($data[0]);
        }
        foreach ($data as $key => $val) {
     ...
     *            to match against
     * @param array $arrv
     *            to be matched against parameter 1
