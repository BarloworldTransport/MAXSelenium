        ...
        if (! file_exists($file)) {
            ...
            if (count($getExt) != 0) {
                ...
                if (($getExt[1] = "xls") || ($getExt[1] = "xlsx")) {
                    ...
                    try {
                        // Get num of arguments passed to function
                        $numargs = func_num_args();
                        // If num of arguments passed greater than 2 then process extra arguments as more sheetnames
                        if ($numargs > 2) {
                            $arg_list = func_get_args();
                            for ($i = 0; $i < $numargs; $i ++) {
                                if ($i != 0) {
                                    $_data[func_get_arg($i)] = $this->readExcelFileData($this->getFileName(), func_get_arg($i));
                                }
                            }
                            if ($_data) {
                                $this->setData($_data);
