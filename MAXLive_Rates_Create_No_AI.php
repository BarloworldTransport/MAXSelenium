...
class MAXLive_Rates_Create extends PHPUnit_Framework_TestCase {
	...
	public function testCreateRates() {
		...
		/** Temporarily commented out to get script working without spreadsheet column validation 
		...
		echo "RATES DATA:" . PHP_EOL;
		var_dump($_ratesData);
		echo "END RATES DATA:" . PHP_EOL;
		
		echo "LOCATIONS DATA:" . PHP_EOL;
		var_dump($_locationsData);
		echo "END LOCATIONS DATA:" . PHP_EOL;

		echo "OFFLOADING CUSTOMERS DATA:" . PHP_EOL;
		var_dump($_offloadingData);
		echo "END OFFLOADING CUSTOMERS DATA:" . PHP_EOL;

		echo "BUSINESS UNIT DATA:" . PHP_EOL;
		var_dump($_buData);
		echo "END BUSINESS UNIT DATA:" . PHP_EOL;
		
		echo "CUSTOMER DATA:" . PHP_EOL;
		var_dump($_customerData);
		echo "END CUSTOMER DATA:" . PHP_EOL;
		
		die("Killing script to list data only...");
		...
		// Change script to use XLS file to use multiple sheets to import rates, locations, offloading customers seperately
		
		// Import data from file and if successful continue script else terminate script and return error
		
		if ( $_locationsData || $_offloadingData || $_ratesData) {
			 ...
			 */
				...
				if ($_locationsData) {
				    foreach ($_locationsData as $_mainKey => $_mainValue) {
				    /*
				    ...
				    $_queries = array ();
				    $_sqlquery = preg_replace("/%n/", $_mainValue['city'], self::SQL_QUERY_LOCATION);
				    $_queries['city'] = preg_replace("/%t/", self::_TYPE_CITY, $_sqlquery);
				    
				    $_sqlquery = preg_replace("/%n/", $_mainValue['point'], self::SQL_QUERY_LOCATION);
				    $_queries['point'] = preg_replace("/%t/", self::_TYPE_POINT, $_sqlquery);;
				    
				    $_sqlquery = preg_replace("/%n/", $_mainValue['city'], self::SQL_QUERY_LOCATION);
				    $_queries['customer_location_link'];
